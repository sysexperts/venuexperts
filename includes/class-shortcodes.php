...
class KH_Shortcodes {
	...
	public function register(): void {
		...
		add_shortcode( 'kh_footer', array( $this, 'footer' ) );
	}
	...
	public function event_highlights( $atts ): string {
		$atts = shortcode_atts(
			array(
				...
				'category'     => '',
				'title'        => 'HIGH-LIGHTS',
				'subtitle'     => 'DAS KULTURHAUS SCHWANEN',
				'text'         => 'Der Schwanen ist ein Kulturhaus, mit allen unterhaltenden und vergnüglichen Aspekten von Kultur, also ein Stück weit ist er auch ein Freizeitcenter für Unterhaltung und Amüsement, eben ein Stück weit. Der Schwanen ist ein Kulturhaus und damit auch ein Stück weit ein Wirtshaus mit Saal für alles mögliche, aber eben nur ein Stück weit.',
	 ...
	 * Shortcode: [kh_footer]
	 *
	 * Professioneller Footer mit allen anpassbaren Elementen.
	 *
	 * @param array $atts Shortcode-Attribute.
	 * @return string HTML-Ausgabe.
	 */
	public function footer( $atts ): string {
		$atts = shortcode_atts(
			array(
				// Kontakt
				...
				// Links Spalte 1
				'links1_title'    => 'Über uns',
				'links1'          => '',
				
				// Links Spalte 2
				'links2_title'    => 'Service',
				'links2'          => '',
				
				// Links Spalte 3
				'links3_title'    => 'Rechtliches',
				'links3'          => 'Impressum|/impressum,Datenschutz|/datenschutz,Barrierefreiheit|/barrierefreiheit',
				...
				// Copyright
				'copyright'       => '© 2026 Kulturhaus Schwanen. Alle Rechte vorbehalten.',
		...
		// Links parsen (Format: "Text|URL,Text2|URL2")
		$parse_links = function( $links_string ) {
			if ( empty( $links_string ) ) {
				return array();
			}
			
			$links = array();
			$items = explode( ',', $links_string );
		...
		$links1 = $parse_links( $atts['links1'] );
		$links2 = $parse_links( $atts['links2'] );
		$links3 = $parse_links( $atts['links3'] );

		ob_start();
		?>
		<footer class="kh-footer" style="background-color: <?php echo esc_attr( $atts['bg_color'] ); ?>; color: <?php echo esc_attr( $atts['text_color'] ); ?>;">
			<div class="kh-footer__container">
				
				<!-- Kontakt -->
				<?php if ( ! empty( $atts['phone'] ) || ! empty( $atts['email'] ) || ! empty( $atts['address'] ) ) : ?>
					<div class="kh-footer__column">
						<h3 class="kh-footer__title"><?php echo esc_html( $atts['contact_title'] ); ?></h3>
						
						<?php if ( ! empty( $atts['phone'] ) ) : ?>
							<p class="kh-footer__contact">
								<strong><?php echo esc_html( $atts['phone_label'] ); ?>:</strong><br>
						...
						<?php if ( ! empty( $atts['email'] ) ) : ?>
							<p class="kh-footer__contact">
								<strong><?php echo esc_html( $atts['email_label'] ); ?>:</strong><br>
						...
						<?php if ( ! empty( $atts['address'] ) ) : ?>
							<p class="kh-footer__contact">
								<strong><?php echo esc_html( $atts['address_label'] ); ?>:</strong><br>
				...
				<?php if ( ! empty( $links1 ) ) : ?>
					<div class="kh-footer__column">
						<h3 class="kh-footer__title"><?php echo esc_html( $atts['links1_title'] ); ?></h3>
				...
				<?php if ( ! empty( $links2 ) ) : ?>
					<div class="kh-footer__column">
						<h3 class="kh-footer__title"><?php echo esc_html( $atts['links2_title'] ); ?></h3>
						<ul class="kh-footer__links">
				...
				<!-- Links Spalte 3 (Rechtliches) -->
				<?php if ( ! empty( $links3 ) ) : ?>
					<div class="kh-footer__column">
						<h3 class="kh-footer__title"><?php echo esc_html( $atts['links3_title'] ); ?></h3>
						<ul class="kh-footer__links">
				...
				<!-- Social Media -->
				<?php if ( ! empty( $atts['facebook'] ) || ! empty( $atts['instagram'] ) || ! empty( $atts['twitter'] ) || ! empty( $atts['youtube'] ) || ! empty( $atts['linkedin'] ) ) : ?>
					<div class="kh-footer__column">
						<h3 class="kh-footer__title"><?php echo esc_html( $atts['social_title'] ); ?></h3>
						<div class="kh-footer__social">
							<?php if ( ! empty( $atts['facebook'] ) ) : ?>
								<a href="<?php echo esc_url( $atts['facebook'] ); ?>" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
							...
							<?php if ( ! empty( $atts['instagram'] ) ) : ?>
								<a href="<?php echo esc_url( $atts['instagram'] ); ?>" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
									<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
							...
							<?php if ( ! empty( $atts['twitter'] ) ) : ?>
								<a href="<?php echo esc_url( $atts['twitter'] ); ?>" target="_blank" rel="noopener noreferrer" aria-label="Twitter">
									<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
							...
							<?php if ( ! empty( $atts['youtube'] ) ) : ?>
								<a href="<?php echo esc_url( $atts['youtube'] ); ?>" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
									<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
							...
							<?php if ( ! empty( $atts['linkedin'] ) ) : ?>
								<a href="<?php echo esc_url( $atts['linkedin'] ); ?>" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
									<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
			...
			<!-- Copyright Bar -->
			<?php if ( ! empty( $atts['copyright'] ) ) : ?>
				<div class="kh-footer__bottom">
					<p class="kh-footer__copyright"><?php echo esc_html( $atts['copyright'] ); ?></p>
				</div>
			<?php endif; ?>
		</footer>
		<?php
